...
// Dynamic cross-platform config discovery
export function findConfigPaths() {
  const home = os.homedir();
  const candidates = [
    path.join(home, '.gemini', 'antigravity-ide', 'mcp_config.json'),
    path.join(home, '.gemini', 'antigravity', 'mcp-config.json'),
  ...
  for (const p of paths) {
    try {
      if (fs.existsSync(p)) {
        ...
        return { config: JSON.parse(content), activePath: p, allPaths: paths };
      }
    } catch (e) {}
  }
  return { config: { mcpServers: {}, disabledMcpServers: {} }, activePath: paths[0], allPaths: paths };
}
...
export function writeConfig(config) {
  ...
  for (const p of paths) {
    try {
      ...
      console.error(`Failed writing config to ${p}:`, e.message);
    }
  }
}

// Package Inspection & Metadata
export function extractPackageInfo(serverName, cfg) {
  if (!cfg) return null;
  ...
  if (cmd.includes('npx')) {
    for (const arg of args) {
      ...
      let clean = arg.split('@latest')[0].split('@')[0];
  ...
  if (cmd.includes('uv') || cmd.includes('uvx') || cmd.includes('python')) {
    ...
    if (serverName.toLowerCase() === 'mempalace') return { type: 'pypi', pkg: 'mempalace', name: serverName };
    if (serverName.toLowerCase() === 'scrapling') return { type: 'pypi', pkg: 'scrapling', name: serverName };
  }

  return { type: 'custom', name: serverName };
}

export function resolveRepoUrl(pkgInfo, pkgData) {
...
export function getLocalPythonVersion(pkg) {
  try {
...
export function fetchLatestNpmVersion(pkg) {
  return new Promise((resolve) => {
    const url = `https://registry.npmjs.org/${encodeURIComponent(pkg)}/latest`;
    https.get(url, { headers: { 'User-Agent': 'mcp-control-panel' } }, (res) => {
      let body = '';
      res.on('data', chunk => body += chunk);
      res.on('end', () => {
        try {
          const json = JSON.parse(body);
          resolve({ version: json.version || null, data: json });
        } catch { resolve(null); }
      });
...
export function fetchLatestPyPiVersion(pkg) {
  return new Promise((resolve) => {
    ...
    https.get(url, { headers: { 'User-Agent': 'mcp-control-panel' } }, (res) => {
      ...
      res.on('end', () => {
        try {
          const json = JSON.parse(body);
          resolve({ version: json.info ? json.info.version : null, data: json.info || {} });
        } catch { resolve(null); }
      });
    }).on('error', () => resolve(null));
  });
...
export function createServer() {
  return http.createServer(async (req, res) => {
    ...
    if (req.url === '/api/mcp' && req.method === 'GET') {
      try {
        const { config, activePath, allPaths } = readConfig();
        ...
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ...config, repoUrls, activePath, allPaths }));
      } catch (e) {
        ...
        try {
          ...
          if (pkgInfo.type === 'npm') {
            ...
            await new Promise((resolve) => {
              exec(`npx --yes ${pkgInfo.pkg}@latest --help`, { timeout: 25000 }, () => resolve());
            });
          } else if (pkgInfo.type === 'pypi') {
            await new Promise((resolve) => {
              exec(`pip install --upgrade ${pkgInfo.pkg}`, { timeout: 35000 }, () => resolve());
            });
          }
...
export function startServer(port = 7890, callback) {
